...
def pygameWrapper():
					...
					for i in range(len(self.squares[row][col].node.adjacentNodes)):
							if self.squares[row][col].node.adjacentNodes[i][0].intersectionName == previous_node:
								if(self.squares[row][col].node.adjacentNodes[i][1] == "W"):
									pygame.draw.line(window, color, (self.squares[row][col].x, self.squares[row][col].y - self.width), (self.squares[row][col].x, self.squares[row][col].y - self.gap + self.width), 3)
									break
								elif(self.squares[row][col].node.adjacentNodes[i][1] == "E"):
									pygame.draw.line(window, color, (self.squares[row][col].x, self.squares[row][col].y + self.width), (self.squares[row][col].x, self.squares[row][col].y + self.gap - self.width), 3)
									break
								elif(self.squares[row][col].node.adjacentNodes[i][1] == "S"):
									pygame.draw.line(window, color, (self.squares[row][col].x + self.width, self.squares[row][col].y), (self.squares[row][col].x + self.gap - self.width, self.squares[row][col].y), 3)
									break
				...
				if event.type == pygame.MOUSEBUTTONDOWN:
					...
					if number_selected == 1:
						...
						if type_of_path != "Both":
							...
							weight_map_shortest, previous_map_shortest = ImTrying.shortest_path_visual(adjacencyList, index, "Shortest")
							weight_map_safest, previous_map_safest = ImTrying.shortest_path_visual(adjacencyList, index, "Safest")
	...
	def main_menu():
		...
		text_font = pygame.font.SysFont("comicsans", 30)
		game_font = pygame.font.SysFont("comicsans", 15)
		run = True
		while run:
			window.fill((128, 128, 128))
			title_label = title_font.render("Welcome to our Project!", 1, (255, 255, 255))
			window.blit(title_label, (WIDTH/2 - title_label.get_width()/2, title_label.get_height())) 

			how_label = title_font.render("How To Play:" , 1, (255, 255, 255))
			window.blit(how_label, (WIDTH/2 - how_label.get_width()/2, title_label.get_height() + how_label.get_height()))
			...
			window.blit(project_info, (WIDTH/2 - project_info.get_width()/2, 3 * title_label.get_height() + project_info.get_height()))
			...
			window.blit(project_info, (WIDTH/2 - project_info.get_width()/2, 3 * title_label.get_height() + 2 * project_info.get_height()))
			...
			window.blit(project_info, (WIDTH/2 - project_info.get_width()/2, 3 * title_label.get_height() + 6 * project_info.get_height()))
			...
			window.blit(project_info, (WIDTH/2 - project_info.get_width()/2, 3 * title_label.get_height() + 4 * project_info.get_height()))

			info = "Press 'b' to show both safest and shortest (Green: Safest; Orange: Shortest)"
			project_info = text_font.render(info, 1, (255, 255, 255))
			window.blit(project_info, (WIDTH/2 - project_info.get_width()/2, 3 * title_label.get_height() + 5 * project_info.get_height()))

			exit_info = "Press 'q' to exit and close window"
			project_info = text_font.render(exit_info, 1, (255, 255, 255))
			window.blit(project_info, (WIDTH/2 - project_info.get_width()/2, 3 * title_label.get_height() + 7 * project_info.get_height()))
			pygame.display.update()
			for event in pygame.event.get():
				if event.type == pygame.QUIT:
					run = False
